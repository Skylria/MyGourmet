...
def restaurantes_view(request):
    ...
    if latitude is not None and longitude is not None:
        ...
        print(f"Restaurantes no Google Maps: {restaurants}")
        restaurants_banco = buscar_restaurantes_banco(latitude, longitude, 5000)
        print(f"Restaurantes no Banco de Dados: {restaurants_banco}")

        # Unir as listas de restaurantes, mantendo apenas os restaurantes válidos
        restaurants = [restaurant for restaurant in restaurants if restaurant.get('name') != 'Desconhecido']
        restaurants += [restaurant for restaurant in restaurants_banco if restaurant.get('name') != 'Desconhecido']
        ...
        return render(request, 'restaurantes_list.html', {
            ...
            'menu_url': menu_url  # Passa a URL do menu
        })
    else:
